...
class YoloPredictor(BasePredictor, QObject):
    ...
    yolo2main_liquidity = Signal(dict)          # Detected target results (number of each category)
    ...
    def postprocess(self, preds, img, orig_img):
        ...
        # Ví dụ: [x1, y1, x2, y2, confidence, class]
            ...
            results.append(Results(orig_img=orig_img, path=img_path, names=self.model.names, boxes=pred))#, tracks=tracks))
        return results
...
class MainWindow(QMainWindow, Ui_MainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.yolo_predict.yolo2main_liquidity.connect(lambda x:self.Class_num.setText(str(x)))     

        self.yolo_predict.yolo2main_target_num.connect(lambda x:self.Target_num.setText(str(x)))  
        self.yolo_predict.yolo2main_target_num.connect(lambda x:self.update_plot_bubble(x))       
    ...
    def update_plot_bubble(self,msg1):
        if self.visual_button.isChecked():
            ...
            self.canvas.axes.plot(self.xdata, self.ydata, color='#08F7FE',label = 'Bubble')
    ...
    def load_rtsp(self, ip):
        try:
            self.stop()
            MessageBox(
                self.close_button, title='Load rtsp', text='Nhap rtsp...', time=1000, auto=True).exec()
